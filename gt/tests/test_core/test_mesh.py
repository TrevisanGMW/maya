...
from gt.core import mesh as core_mesh
...
class TestMeshCore(unittest.TestCase):
    ...
    def test_get_mesh_path(self):
        ...
        result = core_mesh.get_mesh_file_path("qr_code_package_github")
        expected = os.path.join(DataDirConstants.DIR_MESHES,
                                f"qr_code_package_github.{core_mesh.MESH_FILE_EXTENSION}")
        self.assertEqual(expected, result)

    def test_import_obj_file(self):
        result = core_mesh.import_obj_file(self.triangle_file_path)
        expected = ['groupId1', 'lambert2SG', 'lambert2SG1', 'materialInfo1', '|triangle', '|triangle|triangleShape']
        self.assertEqual(expected, result)

    def test_get_vertices(self):
        cube = maya_test_tools.create_poly_cube()
        result = core_mesh.get_vertices(cube)
    ...
    def test_get_vertices_triangle(self):
        core_mesh.import_obj_file(self.triangle_file_path)
        result = core_mesh.get_vertices("|triangle")
        expected = ['triangle.vtx[0]', 'triangle.vtx[1]', 'triangle.vtx[2]']
        self.assertEqual(expected, result)

    def test_mesh_file_init(self):
        mesh_file = core_mesh.MeshFile(file_path=self.triangle_file_path, metadata=None)
    ...
    def test_mesh_file_fail_init(self):
        ...
        mesh_file = core_mesh.MeshFile(file_path="mocked_path", metadata=None)
        self.assertFalse(mesh_file.is_valid())
        logging.disable(logging.NOTSET)

    def test_mesh_file_metadata(self):
        mocked_metadata = {"key": "value"}
        mesh_file = core_mesh.MeshFile(file_path=self.triangle_file_path, metadata=mocked_metadata)
    ...
    def test_get_curve_preview_image_path(self):
        path = core_mesh.get_mesh_preview_image_path("qr_code_package_github")
    ...
    #     cube = maya_test_tools.create_poly_cube()
    #     result = core_mesh.export_obj_file(export_path=export_path, obj_names=cube)
    ...
    def test_export_obj_file_options(self):
        ...
        result = core_mesh.export_obj_file(export_path=export_path, obj_names=cube, options=options)
        self.assertTrue(os.path.exists(result))
        self.assertEqual(export_path, result)
        exported_files = os.listdir(self.temp_dir)
    ...
    def test_is_face_string_valid_strings(self):
        ...
        for input_str in valid_strings:
            with self.subTest(input_str=input_str):
                self.assertTrue(core_mesh.is_face_string(input_str), f"Expected {input_str} to be valid")
    ...
    def test_is_face_string_invalid_strings(self):
        ...
        for input_str in invalid_strings:
            with self.subTest(input_str=input_str):
                self.assertFalse(core_mesh.is_face_string(input_str), f"Expected {input_str} to be invalid")
    ...
    def test_is_edge_string_valid_strings(self):
        ...
        for input_str in valid_strings:
            with self.subTest(input_str=input_str):
                self.assertTrue(core_mesh.is_edge_string(input_str), f"Expected {input_str} to be valid")
    ...
    def test_is_edge_string_invalid_strings(self):
        ...
        for input_str in invalid_strings:
            with self.subTest(input_str=input_str):
                self.assertFalse(core_mesh.is_edge_string(input_str), f"Expected {input_str} to be invalid")
    ...
    def test_is_vertex_string_valid_strings(self):
        ...
        for input_str in valid_strings:
            with self.subTest(input_str=input_str):
                self.assertTrue(core_mesh.is_vertex_string(input_str), f"Expected {input_str} to be valid")
    ...
    def test_is_vertex_string_invalid_strings(self):
        ...
        for input_str in invalid_strings:
            with self.subTest(input_str=input_str):
                self.assertFalse(core_mesh.is_vertex_string(input_str), f"Expected {input_str} to be invalid")

    def test_extract_components_from_face(self):
        cube = maya_test_tools.create_poly_cube()
        result = core_mesh.extract_components_from_face(f'{cube}.f[0]')
