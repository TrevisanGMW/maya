...
from gt.core import uuid as core_uuid
cmds = maya_test_tools.cmds


class TestUUIDCore(unittest.TestCase):
    def setUp(self):
    ...
    def test_short_uuid_generation(self):
        expected_length = 8
        result = core_uuid.generate_uuid(short=True)
        expected = isinstance(result, str) and len(result) == expected_length
        self.assertTrue(expected)

    def test_short_uuid_generation_custom_length(self):
        custom_length = 6
        result = core_uuid.generate_uuid(short=True, short_length=custom_length)
        expected = isinstance(result, str) and len(result) == custom_length
        self.assertTrue(expected)

    def test_short_uuid_invalid_length(self):
        with self.assertRaises(ValueError):
            core_uuid.generate_uuid(short=True, short_length=-1)

    def test_remove_dashes(self):
        result = core_uuid.generate_uuid(remove_dashes=True)
        expected_length = 32
    ...
    def test_generate_uuid_no_conflicting_short(self):
        ...
        for index in range(0, 1000):
            uuids.add(core_uuid.generate_uuid(short=True, short_length=12))
            counter += 1
        self.assertEqual(counter, len(uuids))

    def test_valid_uuid(self):
        valid_uuid = "123e4567-e89b-12d3-a456-426655440000"
        self.assertTrue(core_uuid.is_uuid_valid(valid_uuid))
    ...
    def test_invalid_uuid_close(self):
        ...
        self.assertFalse(core_uuid.is_uuid_valid(valid_uuid))

    def test_valid_short_uuid(self):
        uuid = "abc123"
        result = core_uuid.is_short_uuid_valid(uuid)
        self.assertTrue(result)

    def test_valid_short_uuid_with_length(self):
        uuid = "abc123"
        length = 6
        result = core_uuid.is_short_uuid_valid(uuid, length=length)
        self.assertTrue(result)
    ...
    def test_add_proxy_attribute(self):
        ...
        result = core_uuid.add_uuid_attr(cube, attr_name)
    ...
    def test_add_proxy_attribute_multiple_objects(self):
        ...
        attr_name = "mockedAttrName"
        result = core_uuid.add_uuid_attr([cube_one, cube_two], attr_name)
        expected = [f'{cube_one}.{attr_name}', f'{cube_two}.{attr_name}']
        self.assertEqual(expected, result)

    def test_invalid_object_list_input(self):
        obj_list = 123  # Invalid input
        proxy_attrs = core_uuid.add_uuid_attr(obj_list, 'mockedAttrName')
        expected = []
        self.assertEqual(proxy_attrs, expected)

    def test_invalid_attribute_name(self):
        objects = ['object1', 'object2']
        proxy_attrs = core_uuid.add_uuid_attr(objects, None)  # Invalid attribute name
        expected = []
        self.assertEqual(proxy_attrs, expected)

    @patch('gt.core.uuid.add_attr')
    @patch('gt.core.uuid.set_attr')
    @patch('gt.core.uuid.generate_uuid')
    def test_initial_uuid_generation(self, mock_generate_uuid, mock_set_attr, mock_add_attr):
        ...
        proxy_attrs = core_uuid.add_uuid_attr(objects, 'mockedAttrName', set_initial_uuid_value=True)
    ...
    def test_find_object_with_uuid(self):
        ...
        attr_name = "mockedAttrName"
        created_uuid_attr = core_uuid.add_uuid_attr([cube_one, cube_two], attr_name)
        ...
        result = core_uuid.get_object_from_uuid_attr(uuid_string="mocked_uuid_value", attr_name="mockedAttrName")
        expected = "|pCube1"
        self.assertEqual(expected, result)
    ...
    def test_get_uuid(self):
        ...
        result = core_uuid.get_uuid(cube)
    ...
    def test_get_object_from_uuid(self):
        ...
        result = core_uuid.get_object_from_uuid(_uuid)
