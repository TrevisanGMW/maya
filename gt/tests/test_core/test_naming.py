...
from gt.core import naming as core_naming

cmds = maya_test_tools.cmds


class TestNamingCore(unittest.TestCase):
    def setUp(self):
    ...
    def test_naming_constants(self):
        constant_attributes = vars(core_naming.NamingConstants)
        ...
        for constant_key in constant_keys:
            name_class = getattr(core_naming.NamingConstants, constant_key)
    ...
    def test_get_short_name(self):
        result = core_naming.get_short_name(long_name="grandparent|parent|item")
        expected = "item"
        self.assertEqual(expected, result)

    def test_get_short_name_long(self):
        result = core_naming.get_short_name(long_name="one|two|three|four|five|six|seven|eight|nine|item")
        expected = "item"
        self.assertEqual(expected, result)

    def test_get_long_name(self):
        cube = maya_test_tools.create_poly_cube()
        result = core_naming.get_long_name(cube)
        expected = "|pCube1"
        self.assertEqual(expected, result)
    ...
    def test_get_short_name_short(self):
        result = core_naming.get_short_name(long_name="|item")
        expected = "item"
        self.assertEqual(expected, result)

    def test_get_short_name_short_remove_namespace(self):
        result = core_naming.get_short_name(long_name="|ns:item", remove_namespace=True)
        expected = "item"
        self.assertEqual(expected, result)
    ...
    def test_get_short_name_maya(self):
        ...
        for cube in non_unique_cubes:
            result = core_naming.get_short_name(long_name=cube)
            expected = "cubeShape"
            self.assertEqual(expected, result)
