...
class TestPluginCore(unittest.TestCase):
    ...
    def test_load_plugin(self):
        ...
        result = core_plugin.load_plugin(plugin)
        expected = True
    ...
    def test_load_plugins(self):
        ...
        for plugin in plugins:
            self.assertFalse(maya_test_tools.is_plugin_loaded(plugin))
        result = core_plugin.load_plugins(plugins)
    ...
    def test_unload_plugin(self):
        ...
        self.assertTrue(maya_test_tools.is_plugin_loaded(plugin))
        result = core_plugin.unload_plugin(plugin)
        expected = True
        self.assertEqual(expected, result)
        self.assertFalse(maya_test_tools.is_plugin_loaded(plugin))
    ...
    def test_unload_plugins(self):
        ...
        for plugin in plugins:
            self.assertTrue(maya_test_tools.is_plugin_loaded(plugin))
        result = core_plugin.unload_plugins(plugins)
        expected = [('objExport', True), ('Unfold3D', True)]
        self.assertEqual(expected, result)
        for plugin in plugins:
            self.assertFalse(maya_test_tools.is_plugin_loaded(plugin))
