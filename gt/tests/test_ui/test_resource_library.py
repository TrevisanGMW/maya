...
class TestResourceLibrary(unittest.TestCase):
    def test_get_resource_path(self):
        result = ui_res_lib.get_resource_path(resource_name="name", resource_folder="folder")
        expected = os.path.join("folder", "name")
        self.assertEqual(expected, result)

    def test_get_resource_path_sub_folder(self):
        result = ui_res_lib.get_resource_path(resource_name="name", resource_folder="folder", sub_folder="sub")
        expected = os.path.join("folder", "sub", "name")
        self.assertEqual(expected, result)

    def test_get_icon_path(self):
        result = ui_res_lib.get_icon_path(icon_name="package_logo.svg")
        expected = os.path.join(ui_res_lib.ResourceDirConstants.DIR_ICONS, "package_logo.svg")
        self.assertEqual(expected, result)

    def test_get_font_path(self):
        result = ui_res_lib.get_font_path(font_name="Roboto-Regular.ttf")
        expected = os.path.join(ui_res_lib.ResourceDirConstants.DIR_FONTS, "Roboto-Regular.ttf")
        self.assertEqual(expected, result)

    def test_get_stylesheet_content(self):
        result = ui_res_lib.get_stylesheet_content(stylesheet_name="maya_dialog_base")
        expected = "QWidget"
        self.assertIn(expected, result)

    def test_process_stylesheet_variables(self):
        mocked_variables = {"@original": "modified"}
        result = ui_res_lib.process_stylesheet_variables(
    ...
    def test_color_rgb_black_class(self):
        result = ui_res_lib.Color.RGB.black
        expected = "rgb(0, 0, 0)"
        self.assertEqual(expected, result)

    def test_color_rgb_white_class(self):
        result = ui_res_lib.Color.RGB.white
        expected = "rgb(255, 255, 255)"
        self.assertEqual(expected, result)

    def test_color_hex_black_class(self):
        result = ui_res_lib.Color.Hex.black
        expected = "#000000"
        self.assertEqual(expected, result)

    def test_color_hex_white_class(self):
        result = ui_res_lib.Color.Hex.white
        expected = "#FFFFFF"
        self.assertEqual(expected, result)

    def test_rgb_to_hex_without_alpha(self):
        # Test RGB values without alpha
        self.assertEqual(ui_res_lib.rgba_to_hex(255, 0, 0), "#FF0000")
        self.assertEqual(ui_res_lib.rgba_to_hex(0, 255, 0), "#00FF00")
        self.assertEqual(ui_res_lib.rgba_to_hex(0, 0, 255), "#0000FF")
        self.assertEqual(ui_res_lib.rgba_to_hex(128, 128, 128), "#808080")

    def test_rgba_to_hex_with_alpha(self):
        # Test RGBA values with alpha
        self.assertEqual(ui_res_lib.rgba_to_hex(255, 0, 0, 128, True), "#FF000080")
        self.assertEqual(ui_res_lib.rgba_to_hex(0, 255, 0, 64, True), "#00FF0040")
        self.assertEqual(ui_res_lib.rgba_to_hex(0, 0, 255, 192, True), "#0000FFC0")
        self.assertEqual(ui_res_lib.rgba_to_hex(128, 128, 128, 255, True), "#808080FF")

    def test_rgba_to_hex_without_alpha(self):
        # Test RGBA values without alpha (alpha should default to 255)
        self.assertEqual(ui_res_lib.rgba_to_hex(255, 0, 0, include_alpha=False), "#FF0000")
        self.assertEqual(ui_res_lib.rgba_to_hex(0, 255, 0, include_alpha=False), "#00FF00")
        self.assertEqual(ui_res_lib.rgba_to_hex(0, 0, 255, include_alpha=False), "#0000FF")
        self.assertEqual(ui_res_lib.rgba_to_hex(128, 128, 128, include_alpha=False), "#808080")

    def test_rgb_to_hex(self):
        # Test RGB values without alpha
        self.assertEqual(ui_res_lib.rgb_to_hex(255, 0, 0), "#FF0000")
        self.assertEqual(ui_res_lib.rgb_to_hex(0, 255, 0), "#00FF00")
        self.assertEqual(ui_res_lib.rgb_to_hex(0, 0, 255), "#0000FF")
        self.assertEqual(ui_res_lib.rgb_to_hex(128, 128, 128), "#808080")

    def test_rgb_to_hex_boundary_values(self):
        # Test boundary values
        self.assertEqual(ui_res_lib.rgb_to_hex(0, 0, 0), "#000000")  # Test black
        self.assertEqual(ui_res_lib.rgb_to_hex(255, 255, 255), "#FFFFFF")  # Test white
        self.assertEqual(ui_res_lib.rgb_to_hex(0, 0, 0), "#000000")  # Test black
        self.assertEqual(ui_res_lib.rgb_to_hex(255, 255, 255), "#FFFFFF")  # Test white

    def test_valid_rgb_string(self):
        rgb_string = "rgb(255, 255, 255)"
        result = ui_res_lib.parse_rgb_numbers(rgb_string)
    ...
    def test_invalid_rgba_string(self):
        ...
        result = ui_res_lib.parse_rgb_numbers(rgba_string)
    ...
    def test_empty_string(self):
        result = ui_res_lib.parse_rgb_numbers("")
        self.assertIsNone(result)
    ...
    def test_rgba_string_to_hex(self):
        result = ui_res_lib.rgba_string_to_hex("rgba(255,255,255,255)")
        expected = "#FFFFFF"
        self.assertEqual(expected, result)

    def test_resource_dir_constants(self):

        all_dirs_attributes = vars(ui_res_lib.ResourceDirConstants)
        ...
        for dir_key in all_dirs_keys:
            dir_path = getattr(ui_res_lib.ResourceDirConstants, dir_key)
            if not dir_path:
    ...
    def test_hex_color_pattern(self):
        all_attributes = dir(ui_res_lib.Color.Hex)
        ...
        for hex_color in user_attributes:
            attribute_content = getattr(ui_res_lib.Color.Hex, hex_color)
            match = re.match(r"^#[A-F0-9]{6}(?:[A-F0-9]{2})?$", attribute_content)
            if not match:
                raise Exception(
                    f'"{attribute_content}" (Key: {hex_color}) does not match expected HEX pattern: '
                    f"\n1. Only uppercase characters.\n2. Expected length (6-8 chars)"
                    f'\n3. Start with "#".\n4. No three digit HEX values.'
                )

    def test_rgb_color_pattern(self):
        all_attributes = dir(ui_res_lib.Color.RGB)
        user_attributes = [attr for attr in all_attributes if not (attr.startswith("__") and attr.endswith("__"))]
        pattern = (
            r"^(rgb|rgba)\((25[0-5]|2[0-4]\d|1\d{2}|[1-9]\d|[0-9]),\s*(25[0-5]|2[0-4]\d|1\d{2}|"
            r"[1-9]\d|[0-9]),\s*(25[0-5]|2[0-4]\d|1\d{2}|[1-9]\d|[0-9])(?:,\s*((?:25[0-5]|2[0-4]\d|"
            r"1\d{2}|[1-9]\d|[0-9])|0))?\)$"
        )
        for rgb_color in user_attributes:
            attribute_content = getattr(ui_res_lib.Color.RGB, rgb_color)
            match = re.match(pattern, attribute_content)
            if not match:
                raise Exception(
                    f'"{attribute_content}" does not match expected RGB pattern:'
    ...
    def test_icon_paths_existence(self):
        all_icon_attributes = vars(ui_res_lib.Icon)
        ...
        for icon_key in all_icon_keys:
            icon_path = getattr(ui_res_lib.Icon, icon_key)
            if not icon_path:
    ...
    def test_stylesheet_variables(self):
        all_attributes = dir(ui_res_lib.StylesheetVariables)
        ...
        for stylesheet_key in stylesheet_keys:
            stylesheet_content = getattr(ui_res_lib.StylesheetVariables, stylesheet_key)
            if not isinstance(stylesheet_content, dict):
                raise Exception(
                    f'Stylesheet output should be "dict" but returned "{type(stylesheet_content)}"'
    ...
    def test_stylesheets(self):
        all_attributes = dir(ui_res_lib.Stylesheet)
        stylesheet_keys = [attr for attr in all_attributes if not (attr.startswith("__") and attr.endswith("__"))]

        for stylesheet_key in stylesheet_keys:
            stylesheet_content = getattr(ui_res_lib.Stylesheet, stylesheet_key)
            if not isinstance(stylesheet_content, str):
                raise Exception(
                    f'Stylesheet output should be "str" but returned "{type(stylesheet_content)}"'
                    f' for stylesheet key: "{stylesheet_key}:.'
                )
    ...
    def test_font_paths_existence(self):
        all_icon_attributes = vars(ui_res_lib.Font)
        ...
        for font_key in all_font_keys:
            font_path = getattr(ui_res_lib.Font, font_key)
            if not font_path:
