...
gt_sel_manager = {
    "use_contains_string": False,  # Active Functions
...
def build_gui_selection_manager():
    ...
    window_gui_selection_manager = cmds.window(
        window_name,
    ...
    contains_string_or_not_checkbox = cmds.checkBoxGrp(
        columnWidth2=[120, 1],
        numberOfCheckBoxes=2,
        label1=" Does Contain ",
        label2="  Doesn't Contain",
    ...
    contains_name_text_field = cmds.textField(
        text="_jnt", en=False, enterCommand=lambda x: update_stored_values_and_run(False)
    )

    cmds.separator(h=10, style="none")  # Empty Space

    contains_no_name_text_field = cmds.textField(
        text="endJnt, eye", en=False, enterCommand=lambda x: update_stored_values_and_run(False)
    )
    ...
    contains_type_or_not_checkbox = cmds.checkBoxGrp(
        columnWidth2=[120, 1],
        numberOfCheckBoxes=2,
        label1=" Does Contain ",
        label2="  Doesn't Contain",
    ...
    contains_type_text_field = cmds.textField(
        text="joint", en=False, enterCommand=lambda x: update_stored_values_and_run(False)
    )
    cmds.separator(h=10, style="none")  # Empty Space
    contains_no_type_text_field = cmds.textField(
        text="mesh", en=False, enterCommand=lambda x: update_stored_values_and_run(False)
    )
    ...
    cmds.text("    ")
    use_visibility_state = cmds.checkBox(
        p=visibility_container,
    ...
    cmds.text("    ")
    use_outline_color = cmds.checkBox(
        p=outline_color_container,
        label="",
        value=gt_sel_manager.get("use_outliner_color"),
        cc=lambda x: update_active_items(),
    )

    has_outliner_color_slider_one = cmds.colorSliderGrp(
        en=False,
        label="Using Outliner Color  --->  ",
        rgb=(
            gt_sel_manager.get("stored_outliner_color")[0],
    ...
    cmds.button(
        ...
        bgc=(0.2, 0.2, 0.2),
        w=30,
        c=lambda x: get_color_from_selection(has_outliner_color_slider_one),
        height=10,
        width=40,
    )
    ...
    cmds.text("    ")
    use_no_outline_color = cmds.checkBox(
        p=outline_no_color_container,
        label="",
        value=gt_sel_manager.get("use_no_outliner_color"),
        cc=lambda x: update_active_items(),
    )

    has_no_outliner_color_slider_one = cmds.colorSliderGrp(
        en=False,
        label=" But Not Using Color  --->   ",
        rgb=(
            gt_sel_manager.get("stored_no_outliner_color")[0],
    ...
    cmds.button(
        ...
        bgc=(0.2, 0.2, 0.2),
        w=30,
        c=lambda x: get_color_from_selection(has_no_outliner_color_slider_one),
        height=10,
        width=40,
    )
    ...
    cmds.button(
        p=target_container, l="-", bgc=(0.5, 0.3, 0.3), w=30, c=lambda x: selection_storage_manager("remove", 1)
    )
    store_sel_btn_one = cmds.button(
        p=target_container,
        l="Store Selection",
        bgc=(0.2, 0.2, 0.2),
        w=91,
    ...
    cmds.button(p=target_container, l="+", bgc=(0.3, 0.5, 0.3), w=30, c=lambda x: selection_storage_manager("add", 1))
    ...
    target_container = cmds.rowColumnLayout(p=body_column, numberOfRows=1)
    cmds.button(
        p=target_container, l="-", bgc=(0.5, 0.3, 0.3), w=30, c=lambda x: selection_storage_manager("remove", 2)
    )
    store_sel_btn_two = cmds.button(
        p=target_container,
        l="Store Selection",
        bgc=(0.2, 0.2, 0.2),
        w=91,
    ...
    cmds.button(p=target_container, l="+", bgc=(0.3, 0.5, 0.3), w=30, c=lambda x: selection_storage_manager("add", 2))
    ...
    save_as_quick_selection_rb1 = cmds.radioButton(
        p=save_as_container, sl=True, label=" Save as Quick Selection  ", cc=lambda x: update_active_items()
    )
        ...
        if len(selection) > 0:
            ...
            if len(obj_attr_list) > 0 and "outlinerColor" in obj_attr_list and "useOutlinerColor" in obj_attr_list:
                ...
                if cmds.getAttr(selection[0] + ".useOutlinerColor"):
                    ...
                    cmds.colorSliderGrp(color_slider, e=True, rgb=extracted_color[0])
                    cmds.warning(
                        "Color extracted, but it looks like the object " "selected is not using a custom outliner color"
                    )
            else:
                cmds.warning("Something went wrong. Try selecting another object.")
        else:
            cmds.warning(
                "Nothing Selected. Please select an object containing the "
    ...
    def update_active_items():
        ...
        gt_sel_manager["use_contains_no_string"] = cmds.checkBoxGrp(
            contains_string_or_not_checkbox, q=True, value2=True
        )
        ...
        gt_sel_manager["stored_save_as_quick_selection"] = cmds.radioButton(
            save_as_quick_selection_rb1, q=True, select=True
        )

        gt_sel_manager["stored_outliner_color"] = cmds.colorSliderGrp(has_outliner_color_slider_one, q=True, rgb=True)

        gt_sel_manager["stored_no_outliner_color"] = cmds.colorSliderGrp(
            has_no_outliner_color_slider_one, q=True, rgb=True
        )
    ...
    icon = ui_qt.QtGui.QIcon(ui_res_lib.Icon.tool_selection_manager)
    ...
    for obj in selection:
        ...
        if gt_sel_manager.get("use_contains_type"):
            for string in gt_sel_manager.get("stored_contains_type"):
                ...
                if (
                    gt_sel_manager.get("stored_shape_node_type") == "Select Both Parent and Shape"
                    and string in obj_shape_type
                    or string in obj_type
                ):
                    to_add_list.append(obj)
        ...
        obj_attr_list = []
        if (
            gt_sel_manager.get("use_visibility_state") is True
            or gt_sel_manager.get("use_outliner_color") is True
            or gt_sel_manager.get("use_no_outliner_color") is True
        ):
        ...
        if (
            gt_sel_manager.get("use_visibility_state") is True
            and gt_sel_manager.get("stored_visibility_state") is False
        ):
            if len(obj_attr_list) > 0 and "visibility" in obj_attr_list:
                if cmds.getAttr(obj + ".visibility"):
                    to_remove_list.append(obj)
        ...
        if gt_sel_manager.get("use_outliner_color"):
            if len(obj_attr_list) > 0 and "outlinerColor" in obj_attr_list and "useOutlinerColor" in obj_attr_list:
                ...
                if (
                    outliner_color_r == stored_outliner_color_r
                    and outliner_color_g == stored_outliner_color_g
                    and outliner_color_b == stored_outliner_color_b
                ):
                    to_add_list.append(obj)
        ...
        if gt_sel_manager.get("use_no_outliner_color"):
            if len(obj_attr_list) > 0 and "outlinerColor" in obj_attr_list and "useOutlinerColor" in obj_attr_list:
                ...
                if (
                    outliner_color_r == stored_no_outliner_color_r
                    and outliner_color_g == stored_no_outliner_color_g
                    and outliner_color_b == stored_no_outliner_color_b
                ):
                    to_remove_list.append(obj)
...
def stored_list_manager(stored_list):
    ...
    if missing_elements:
        ...
        cmds.inViewMessage(amg="Stored elements have been selected.", pos="botLeft", fade=True, alpha=0.9)
    if stored_list:
...
def selection_feedback(number_objects):
    ...
    if number_objects != 0:
        ...
        cmds.inViewMessage(amg="No objects were selected.", pos="botLeft", fade=True, alpha=0.9)
