...
# Script Name
script_name = "Add Sine Attributes"
...
def build_gui_add_sine_attr():
    ...
    # Build UI
    window_add_sine_attr = cmds.window(
        window_name,
    ...
    cmds.button(l="Add Sine Attributes", bgc=(0.6, 0.6, 0.6), c=lambda x: validate_operation())
    cmds.separator(h=10, style="none")  # Empty Space
    ...
    icon = ui_qt.QtGui.QIcon(":/sineCurveProfile.png")
    ...
    def validate_operation():
        ...
        if is_valid:
            ...
            if not is_valid:
                cmds.warning(
                    "The object selected has conflicting attributes. "
        ...
        if is_valid:
            if stretchy_name:
                add_sine_attributes(
                    target,
                    sine_prefix=stretchy_name,
                    tick_source_attr="time1.outTime",
                    hide_unkeyable=False,
                    add_absolute_output=add_abs_output_value,
