...
def build_gui_create_testing_keys():
    ...
    window_gui_create_testing_keys = cmds.window(
        window_name,
    ...
    cmds.rowColumnLayout(
        nc=4, cw=[(1, 20), (2, 63), (3, 63), (4, 63)], cs=[(1, 6), (2, 0), (3, 2), (4, 2)], p=body_column
    )
    ...
    cmds.button(l="Reset All Offset Values", bgc=(0.3, 0.3, 0.3), c=lambda x: reset_offset_values())
    cmds.separator(h=10, style="none", p=body_column)  # Empty Space
    ...
    interval_text_field = cmds.floatSliderGrp(
        ...
        fieldMaxValue=1000,
        value=5,
    )  # , cc=lambda args: update_grade_output())

    cmds.separator(h=7, style="none", p=body_column)  # Empty Space
    cmds.rowColumnLayout(nc=1, cw=[(1, 210)], cs=[(1, 10)], p=body_column)
    cmds.button(l="Delete All Keyframes in the Scene", bgc=(0.3, 0.3, 0.3), c=lambda x: gtu_delete_keyframes())

    cmds.separator(h=7, style="none")  # Empty Space

    cmds.button(l="Create Testing Keyframes", bgc=(0.6, 0.6, 0.6), c=lambda x: validate_operation())
    ...
    def validate_operation():
        ...
        try:
            ...
            offset_text_fields = [
                tx_offset_text_field,
                ty_offset_text_field,
                tz_offset_text_field,
                rx_offset_text_field,
                ry_offset_text_field,
                rz_offset_text_field,
                sx_offset_text_field,
                sy_offset_text_field,
                sz_offset_text_field,
            ]
    ...
    icon = ui_qt.QtGui.QIcon(":/setMaxInfluence.png")
...
def build_gui_help_create_testing_keys():
    ...
    cmds.text(
        l='These are the values that will be added to the object.\nIf set to "0.0" it will be ignored. '
        "(No keys will be created)",
        align="left",
    )
    cmds.separator(h=15, style="none")  # Empty Space
    ...
    cmds.text(
        l='Auto creates another key with the inverted offset value.\nFor example, an offset of "1.0"'
        ' will also create another\noffset at "-1.0" creating an oscillating movement.',
        align="left",
    )
    cmds.separator(h=15, style="none")  # Empty Space
    cmds.text(l="Delete Previously Created Keys:", align="left", fn="boldLabelFont")
    cmds.text(
        l="Deletes all keys attached to the selected controls before\ncreating new ones. "
