...
# Settings
settings = {"outliner_color": [0.5, 1, 0.4]}
...
def build_gui_add_offset_transform():
    ...
    # Build UI
    window_gui_generate_inbetween = cmds.window(
        window_name,
    ...
    color_slider = cmds.colorSliderGrp(
        ...
        rgb=(settings.get("outliner_color")[0], settings.get("outliner_color")[1], settings.get("outliner_color")[2]),
        columnWidth=((1, 85), (3, 130)),
    ...
    desired_tag = cmds.textField(
        p=bottom_container,
        text="_offset",
        enterCommand=lambda x: create_inbetween(
    ...
    icon = ui_qt.QtGui.QIcon(":/hsGraphMaterial.png")
...
def create_inbetween(layer_tag, parent_type, layer_type):
    ...
    for obj in selection:
        ...
        cmds.setAttr(
            inbetween_transform + ".outlinerColor",
            settings.get("outliner_color")[0],
