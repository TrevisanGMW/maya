...
def build_gui_transfer_transforms():
    ...
    window_gui_transfer_transforms = cmds.window(
        window_name,
    ...
    cmds.rowColumnLayout(
        nc=3, cw=[(1, 10), (2, 170), (3, 50)], cs=[(1, 10), (2, 0), (3, 0)], p=content_main
    )  # Title Column
    ...
    # Translate
    translate_x_checkbox = cmds.checkBoxGrp(
        columnWidth2=transform_column_width,
    ...
    translate_y_checkbox = cmds.checkBoxGrp(
        columnWidth2=transform_column_width,
        numberOfCheckBoxes=2,
        label1="  Translate Y",
        label2="Invert Value",
    ...
    translate_z_checkbox = cmds.checkBoxGrp(
        columnWidth2=transform_column_width,
        numberOfCheckBoxes=2,
        label1="  Translate Z",
        label2="Invert Value",
    ...
    rotate_x_checkbox = cmds.checkBoxGrp(
        columnWidth2=transform_column_width,
        numberOfCheckBoxes=2,
        label1="  Rotate X",
        label2="Invert Value",
    ...
    rotate_y_checkbox = cmds.checkBoxGrp(
        columnWidth2=transform_column_width,
        numberOfCheckBoxes=2,
        label1="  Rotate Y",
        label2="Invert Value",
    ...
    rotate_z_checkbox = cmds.checkBoxGrp(
        columnWidth2=transform_column_width,
        numberOfCheckBoxes=2,
        label1="  Rotate Z",
        label2="Invert Value",
    ...
    # Scale
    cmds.rowColumnLayout(nc=1, cw=[(1, 230)], cs=[(1, 20)], p=body_column)
    scale_x_checkbox = cmds.checkBoxGrp(
        columnWidth2=transform_column_width,
        numberOfCheckBoxes=2,
        label1="  Scale X",
        label2="Invert Value",
    ...
    scale_y_checkbox = cmds.checkBoxGrp(
        columnWidth2=transform_column_width,
        numberOfCheckBoxes=2,
        label1="  Scale Y",
        label2="Invert Value",
    ...
    scale_z_checkbox = cmds.checkBoxGrp(
        columnWidth2=transform_column_width,
        numberOfCheckBoxes=2,
        label1="  Scale Z",
        label2="Invert Value",
    ...
    cmds.rowColumnLayout(nc=2, cw=[(1, 100), (2, 100)], cs=[(1, 15), (2, 0)], p=body_column)
    left_tag_text_field = cmds.textField(text="left_", enterCommand=lambda x: transfer_transforms_side_to_side("left"))
    right_tag_text_field = cmds.textField(
        text="right_", enterCommand=lambda x: transfer_transforms_side_to_side("right")
    )
    ...
    cmds.button(l="Transfer (Source/Targets)", bgc=(0.6, 0.6, 0.6), c=lambda x: transfer_transforms())
    cmds.separator(h=5, style="none")  # Empty Space
    ...
    cmds.rowColumnLayout(
        nc=4, cw=[(1, 21), (2, 63), (3, 62), (4, 62)], cs=[(1, 15), (2, 0), (3, 3), (4, 3)], p=content_main
    )

    cmds.text(" ")
    cmds.text("X", bgc=[0.5, 0, 0])
    cmds.text("Y", bgc=[0, 0.5, 0])
    cmds.text("Z", bgc=[0, 0, 0.5])
                    ...
                    if transform[0]:  # Using Transform?
                        for target in targets:
                            if not cmds.getAttr(f"{target}.{transform[2]}", lock=True):
                                if transform[1]:  # Inverted?
                                    cmds.setAttr(
                                        f"{target}.{transform[2]}",
                                        (float(gt_transfer_transforms_dict.get(transform[2]) * -1)),
                                    )
                                else:
                                    cmds.setAttr(
                                        f"{target}.{transform[2]}", float(gt_transfer_transforms_dict.get(transform[2]))
                                    )
                            else:
                                errors.append(target + " (" + transform[2] + ") is locked.")
    ...
    icon = ui_qt.QtGui.QIcon(ui_res_lib.Icon.tool_transfer_transforms)
...
def export_trs_transforms():
    ...
    if is_valid:
        file_name = (
            cmds.fileDialog2(
                fileFilter=script_name + " - JSON File (*.json)",
                dialogStyle=2,
                okCaption="Export",
                caption="Exporting TRS for Selected Objects",
            )
            or []
        )
        if len(file_name) > 0:
...
def import_trs_transforms():
    ...
    file_name = (
        cmds.fileDialog2(
            ...
            fileMode=1,
            okCaption="Import",
            caption='Importing Transforms for "' + script_name + '"',
        )
        or []
    )
        ...
        try:
            with open(pose_file) as json_file:
                ...
                try:
                    ...
                    if failed_imports:
                        cmds.warning(
                            "Not all transforms were imported, because not all objects were found. "
                    ...
                    logger.info(str(e))
                    cmds.warning(
                        "An error occurred when importing the pose. "
