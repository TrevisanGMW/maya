...
def blends_to_attr(
    ...
    undesired_filter_strings,
    desired_method="includes",
    undesired_method="includes",
    sort_attr=True,
    ignore_connected=True,
    add_separator=True,
    ignore_case=True,
    ...
    for target in accessible_and_desired_blends:
        if modify_range:
            if target not in current_attributes:
                cmds.addAttr(attr_holder, ln=target, at="double", k=True, maxValue=new_max, minValue=new_min)
            else:
                cmds.warning(
                    '"' + target + '" already existed on attribute holder. '
...
def build_gui_morphing_attributes():
    ...
    def object_load_handler(operation):
        ...
        def failed_to_load_target(failed_message="Failed to Load"):
            cmds.button(attr_holder_status, l=failed_message, e=True, bgc=(1, 0.4, 0.4), w=130)
            ...
            if cmds.objExists(current_selection[0]):
                ...
                if not blendshape_nodes:
                    ...
                    cmds.button(
                        ...
                        l=morphing_attr_settings.get("morphing_obj"),
                        e=True,
                        bgc=(0.6, 0.8, 0.6),
                        w=130,
                    )
        ...
        if operation == "attr_holder":
            ...
            if len(current_selection) == 0:
                ...
                cmds.button(
                    attr_holder_status, l=morphing_attr_settings.get("attr_holder"), e=True, bgc=(0.6, 0.8, 0.6), w=130
                )
            else:
    ...
    def validate_operation():
        ...
        try:
            blend_attr_list = blends_to_attr(
                blend_node,
                attr_holder,
                desired_strings,
                undesired_strings,
    ...
    attr_holder_status = cmds.button(
        ...
        bgc=(0.2, 0.2, 0.2),
        w=130,
        c=lambda x: select_existing_object(morphing_attr_settings.get("attr_holder")),
    )
    ...
    width_two = width_one
    cmds.rowColumnLayout(
        nc=2, cw=[(1, width_one), (2, width_two)], cs=[(1, spacing_one), (2, spacing_two)], p=content_main
    )
    ignore_case_chk = cmds.checkBox(
        "Ignore Uppercase", cc=update_settings, value=morphing_attr_settings.get("ignore_case")
    )
    add_separator_chk = cmds.checkBox(
        "Add Separator", cc=update_settings, value=morphing_attr_settings.get("add_separator")
    )
    cmds.separator(h=7, style="none")  # Empty Space
    cmds.rowColumnLayout(
        nc=2, cw=[(1, width_one), (2, width_two)], cs=[(1, spacing_one), (2, spacing_two)], p=content_main
    )
    ignore_connected_chk = cmds.checkBox(
        "Ignore Connected", cc=update_settings, value=morphing_attr_settings.get("ignore_connected")
    )
    sort_chk = cmds.checkBox("Sort Attributes", cc=update_settings, value=morphing_attr_settings.get("sort_attr"))
    cmds.separator(h=7, style="none")  # Empty Space

    cmds.rowColumnLayout(
        nc=2, cw=[(1, width_one), (2, width_two)], cs=[(1, spacing_one), (2, spacing_two)], p=content_main
    )
    modify_range_chk = cmds.checkBox(
        "Modify Range", cc=update_settings, value=morphing_attr_settings.get("modify_range")
    )
    delete_instead_chk = cmds.checkBox(
        "Delete Instead", cc=update_settings, value=morphing_attr_settings.get("delete_instead")
    )
    ...
    cmds.button(l="Create Morphing Attributes", bgc=(0.6, 0.6, 0.6), c=lambda x: validate_operation())
    cmds.separator(h=10, style="none")  # Empty Space
    ...
    icon = ui_qt.QtGui.QIcon(":/blendShape.png")
