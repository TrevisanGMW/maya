...
def build_gui_world_space_baker():
    ...
    def object_load_handler():
        ...
        if received_valid_element:
            ...
            cmds.button(selection_status_btn, l=load_message, e=True, bgc=(0.6, 0.8, 0.6))
            cmds.button(ws_anim_extract_btn, e=True, en=True)
            cmds.rowColumnLayout(range_column, e=True, en=True)

        else:
            cmds.button(selection_status_btn, l="Failed to Load", e=True, bgc=(1, 0.4, 0.4))
        ...
        if operation == "extract":
            ...
            if result:
                ...
                if len(gt_world_space_baker_settings.get("stored_elements")) != 1:
                    plural = "objects"
                message = (
                    str(len(gt_world_space_baker_settings.get("stored_elements")))
                    + " "
                    + plural
                    + " stored. Frames: "
                    + str(gt_world_space_baker_settings.get("start_time_range"))
                    + "-"
                    + str(gt_world_space_baker_settings.get("end_time_range"))
                )
    ...
    # Build UI
    window_gui_world_space_baker = cmds.window(
        window_name,
    ...
    selection_status_btn = cmds.button(
        l="Not loaded yet",
        bgc=(0.2, 0.2, 0.2),
        w=115,
    ...
    auto_key_start_int_field = cmds.intField(
        value=gt_world_space_baker_settings.get("start_time_range"), cc=lambda x: update_stored_settings()
    )
    cmds.button(l="Get", c=lambda x: get_auto_key_current_frame(), h=5)  # L
    cmds.text("End:")
    auto_key_end_int_field = cmds.intField(
        value=gt_world_space_baker_settings.get("end_time_range"), cc=lambda x: update_stored_settings()
    )
    cmds.button(l="Get", c=lambda x: get_auto_key_current_frame("end"), h=5)  # L
    cmds.separator(h=10, style="none")  # Empty Space
    cmds.rowColumnLayout(nc=1, cw=[(1, 240)], cs=[(1, 20)], p=content_main)
    cmds.separator(h=7, style="none")  # Empty Space
    ws_anim_extract_btn = cmds.button(
        l="Extract World Space", bgc=(0.3, 0.3, 0.3), c=lambda x: validate_operation(), en=False
    )
    cmds.separator(h=7, style="none")  # Empty Space
    ...
    ws_anim_bake_btn = cmds.button(
        l="Bake World Space", bgc=(0.3, 0.3, 0.3), c=lambda x: validate_operation("bake"), en=False
    )
    ...
    icon = ui_qt.QtGui.QIcon(ui_res_lib.Icon.tool_world_space_baker)
...
def select_existing_objects(stored_list):
    ...
    if missing_elements:
        cmds.inViewMessage(
            amg='Some elements are <span style="color:#FF0000;">missing!</span>', pos="botLeft", fade=True, alpha=0.9
        )
        cmds.inViewMessage(amg="Open script editor for more information.", pos="botLeft", fade=True, alpha=0.9)
    else:
                    ...
                    try:
                        ...
                        if "translate" in attr or "rotate" in attr:
                            ...
                            if attr.split(".")[-1].startswith("translate"):
                                gt_world_space_baker_anim_storage["{}.{}".format(obj, "translate")] = (
                                    frame_translate_values
                                )
...
def bake_world_space_data():
    ...
    if is_valid:
        try:
            cmds.refresh(suspend=True)
            cmds.undoInfo(openChunk=True, chunkName="World Space Bake")
