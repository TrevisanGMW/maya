...
def build_gui_auto_fk():
    ...
    window_gui_auto_fk = cmds.window(
        window_name,
    ...
    check_boxes_one = cmds.checkBoxGrp(
        columnWidth2=[120, 1],
        numberOfCheckBoxes=2,
        label1="Mimic Hierarchy",
        label2="Constraint Joint    ",
        v1=gt_auto_fk_settings.get("mimic_hierarchy"),
        v2=gt_auto_fk_settings.get("constraint_joint"),
        cc1=lambda x: set_persistent_settings_auto_fk(
            "gt_auto_fk_mimic_hierarchy", cmds.checkBoxGrp(check_boxes_one, q=True, value1=True)
        ),
        cc2=lambda x: set_persistent_settings_auto_fk(
            "gt_auto_fk_constraint_joint", cmds.checkBoxGrp(check_boxes_one, q=True, value2=True)
        ),
    )

    cmds.rowColumnLayout(nc=1, cw=[(1, 270)], cs=[(1, 13)], p=body_column)

    check_boxes_two = cmds.checkBoxGrp(
        columnWidth2=[120, 1],
        numberOfCheckBoxes=2,
        label1="Colorize Controls",
        label2="Select Hierarchy  ",
        v1=gt_auto_fk_settings.get("auto_color_ctrls"),
        v2=gt_auto_fk_settings.get("select_hierarchy"),
        cc1=lambda x: set_persistent_settings_auto_fk(
            "gt_auto_fk_auto_color_ctrls", cmds.checkBoxGrp(check_boxes_two, q=True, value1=True)
        ),
        cc2=lambda x: set_persistent_settings_auto_fk(
            "gt_auto_fk_select_hierarchy", cmds.checkBoxGrp(check_boxes_two, q=True, value2=True)
        ),
    )
    ...
    ctrl_curve_radius_slider_grp = cmds.floatSliderGrp(
        ...
        label="Curve Radius: ",
        field=True,
        value=float(gt_auto_fk_settings.get("curve_radius")),
        cc=lambda x: set_persistent_settings_auto_fk(
            "gt_auto_fk_curve_radius", str(cmds.floatSliderGrp(ctrl_curve_radius_slider_grp, q=True, value=True))
        ),
    ...
    if (
        gt_auto_fk_settings.get("using_custom_curve") is True
        and gt_auto_fk_settings.get("failed_to_build_curve") is False
    ):
        cmds.button("gt_auto_fk_custom_curve_btn", e=True, l="ACTIVE - Custom Curve", bgc=[0, 0.1, 0])
    ...
    jnt_suffix_tf = cmds.textField(
        text=gt_auto_fk_settings.get("string_joint_suffix"),
        enterCommand=lambda x: generate_fk_controls(),
        cc=lambda x: set_persistent_settings_auto_fk(
            "gt_auto_fk_string_joint_suffix", cmds.textField(jnt_suffix_tf, q=True, text=True)
        ),
    )
    ctrl_suffix_tf = cmds.textField(
        text=gt_auto_fk_settings.get("string_ctrl_suffix"),
        enterCommand=lambda x: generate_fk_controls(),
        cc=lambda x: set_persistent_settings_auto_fk(
            "gt_auto_fk_string_ctrl_suffix", cmds.textField(ctrl_suffix_tf, q=True, text=True)
        ),
    )
    ctrl_grp_suffix_tf = cmds.textField(
        text=gt_auto_fk_settings.get("string_ctrl_grp_suffix"),
        enterCommand=lambda x: generate_fk_controls(),
        cc=lambda x: set_persistent_settings_auto_fk(
            "gt_auto_fk_string_ctrl_grp_suffix", cmds.textField(ctrl_grp_suffix_tf, q=True, text=True)
        ),
    )
    ...
    undesired_strings_text_field = cmds.textField(
        text=gt_auto_fk_settings.get("undesired_strings"),
        enterCommand=lambda x: generate_fk_controls(),
        cc=lambda x: set_persistent_settings_auto_fk(
            "gt_auto_fk_undesired_strings", cmds.textField(undesired_strings_text_field, q=True, text=True)
        ),
    )
    cmds.rowColumnLayout(nc=1, cw=[(1, 260)], cs=[(1, 0)], p=body_column)
    ...
    cmds.separator(h=10)
    cmds.separator(h=10, style="none")  # Empty Space
    cmds.button(l="Generate", bgc=(0.6, 0.6, 0.6), c=lambda x: generate_fk_controls())
    cmds.separator(h=10, style="none")  # Empty Space
            ...
            for jnt in selected_joints:
                ...
                if gt_auto_fk_settings.get("using_custom_curve"):
                    ...
                    try:
                        ...
                        logging.debug(str(e))
                        ctrl = cmds.circle(
                            name=ctrl_name, normal=[1, 0, 0], radius=ctrl_curve_radius, ch=False
        ...
        if result == "Use Python":
            if cmds.promptDialog(query=True, text=True) != "":
                ...
                set_persistent_settings_auto_fk(
                    "gt_auto_fk_custom_curve", str(cmds.promptDialog(query=True, text=True))
                )
            ...
            gt_auto_fk_settings["custom_curve"] = (
                "cmds.curve(p=[[-0.5, 0.5, 0.5], [0.5, 0.5, 0.5], "
            ...
            gt_auto_fk_settings["custom_curve"] = (
                "cmds.curve(p=[[0.0, 0.0, 0.0], [0.0, 4.007, 0.0], "
    ...
    icon = ui_qt.QtGui.QIcon(":/kinInsert.png")
...
def build_gui_help_auto_fk():
    ...
    cmds.text(l='controls. Use the script "Generate Python Curve"', align="left")
...
def create_custom_curve(curve_py_code):
    ...
    try:
        ...
        set_persistent_settings_auto_fk("gt_auto_fk_using_custom_curve", False)
        cmds.button("gt_auto_fk_custom_curve_btn", e=True, l="ERROR - Custom Curve", bgc=[0.3, 0, 0])
